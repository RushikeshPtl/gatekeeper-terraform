...
locals {
  lambda_source_path = "${path.module}${var.lambda_relative_path}functions/${var.function_name}"
  lambda_output_path = "${path.module}${var.lambda_relative_path}functions/${var.function_name}.zip"
}

resource "random_uuid" "lambda_src_hash" {
  keepers = {
    for filename in setunion(
      fileset(local.lambda_source_path, "*.py"),
      fileset(local.lambda_source_path, "requirements.txt"),
      fileset(local.lambda_source_path, "core/**/*.py")
    ):
    filename => filemd5("${local.lambda_source_path}/${filename}")
  }
}

resource "null_resource" "install_dependencies" {
  provisioner "local-exec" {
    command = "pip install -r ${local.lambda_source_path}/requirements.txt -t ${local.lambda_source_path}/ --upgrade"
  }

  triggers = {
    dependencies_versions = filemd5("${local.lambda_source_path}/requirements.txt")
  }
}
