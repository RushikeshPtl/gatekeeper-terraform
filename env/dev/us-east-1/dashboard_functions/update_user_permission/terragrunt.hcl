...
dependency "update_permission_json" {
    config_path = "../update_permission_json"
    mock_outputs_allowed_terraform_commands = ["init", "fmt", "validate", "plan", "show"]
    mock_outputs_merge_strategy_with_state  = "shallow"
    mock_outputs = {
        invoke_arn    = "test"
...
dependency "get_permissions" {
    config_path = "../get_permissions"
    mock_outputs_allowed_terraform_commands = ["init", "fmt", "validate", "plan", "show"]
    mock_outputs_merge_strategy_with_state  = "shallow"
    mock_outputs = {
        invoke_arn    = "test"
...
inputs = {
    ...
    function_name        = "update_user_permission"
    ...
    environment_variables = {
        ...
        "GET_ROLE_PERMISSIONS"          = dependency.get_permissions.outputs.resource_arn
        "UPDATE_PERMISSION_JSON_ARN"    = dependency.update_permission_json.outputs.resource_arn
    }
    layers = [
    ...
    api_paths = [
        {
            "method" = "POST",
            "path"   = "/permissions/update-user-permisson"
        }
    ]
}
